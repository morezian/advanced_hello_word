...
from app.src.data_loader.trade_loader.angular_loader import *
from collections import defaultdict


class LoadData:
    def __init__(self, to_angular=False):
        self.__to_angular = to_angular
        self.__csv_loader = CsvLoader()
        if to_angular == False:
            self.__all_tel_loader = TelegramLoader(
                token="1483369722:AAFQJOLnQeKZd5QjRD4wiI6pfAqoOu-m0Rk", id="-444966767")
            self.__super_tel_loader = TelegramLoader(
                token="1370460089:AAHda25aodvumQO98eR50tnvVouohJ3_VlY", id="-412898183", is_special=True)
            self.__strong_tel_loader = TelegramLoader(
                token="1486040039:AAEDA2JVfpu14VbCWhYdV8mShlyYYoGHPrU", id="-408293635", is_special=True)
            self.__vip_loader = TelegramLoader(
                token="1433676948:AAGJuVmoDH9jsdPYJ1-iEi3JK3Bivl3HhzM", id="-348825048", is_special=True)
        else:
            #start_server = websockets.serve(hello, "localhost", 8765)

            # asyncio.get_event_loop().run_until_complete(start_server)
            # asyncio.get_event_loop().run_forever()
            # AngularLoader(signal_type = "all")
            self.__all_tel_loader = self.__csv_loader
            self.__super_tel_loader = AngularLoader(signal_type="all")
            # self.__super_tel_loader.start()
            # AngularLoader(signal_type = "all")
            self.__strong_tel_loader = self.__super_tel_loader
            # AngularLoader (signal_type = "all")
            self.__vip_loader = self.__super_tel_loader
    ...
    def __get_loader_list(self, stock: Stock, vip_stock_list):
        ...
        last_buy_sell_status = self.__stock_name2last_sent_buy_sell_status.get(
            stock.name)
    ...
    def load(self):
        ...
        for loader, stock_list in loader2stock_list.items():
            ...
            for i in range(len(stock_list)//Factor + 1):
                tmp_list = stock_list[i *
                                      Factor:min((i+1)*Factor, len(stock_list))]
                if len(tmp_list):
